...
async function fixColumnS() {
  try {
    console.log("Applying robust ARRAYFORMULA to S8...");
    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: 'CarbonInput!S8',
      valueInputOption: 'USER_ENTERED',
      requestBody: {
        values: [['=ARRAYFORMULA(IF(ISBLANK(J8:J), "", IFERROR(VALUE(REGEXREPLACE(TO_TEXT(J8:J), "[^0-9.]", "")) * 0.001, "")))']]
      }
    });

    console.log("Successfully fixed S column formula!");
  } catch (err) {
